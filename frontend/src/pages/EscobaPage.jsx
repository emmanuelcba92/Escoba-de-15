...
import { Users, User as UserIcon, Globe, Wifi, ShieldCheck, Wind, ArrowLeft, Home } from 'lucide-react';

function EscobaPage() {
    const [gameStarted, setGameStarted] = useState(false);
    return (
        <ErrorBoundary>
            <EscobaContent gameStarted={gameStarted} setGameStarted={setGameStarted} />
        </ErrorBoundary>
    );
}
        ...
        <>
            ...
            <div className="flex-1 h-full relative">
                <div className="absolute top-4 left-4 sm:top-6 sm:left-6 z-50 flex items-center gap-2">
                    <button onClick={onExit} className="text-white/40 hover:text-white transition-colors flex items-center gap-1 sm:gap-2 font-bold text-[10px] sm:text-sm bg-black/20 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl backdrop-blur-md border border-white/10">
                        &larr; ABANDONAR
                    </button>
                    <Link to="/" className="text-white/40 hover:text-white transition-colors font-bold text-[10px] sm:text-sm bg-black/20 p-1.5 sm:p-2 rounded-xl backdrop-blur-md border border-white/10">
                        <Home size={16} />
                    </Link>
                </div>
                <Board
